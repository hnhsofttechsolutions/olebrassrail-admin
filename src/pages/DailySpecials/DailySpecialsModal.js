...
const DailySpecialsModal = ({ open,setOpen, handleClose,user }) => {
    ...
    const UpcomingEvent = () => {
        ...
        try {
          ...
          Data.append("tag_name","Daily Specials")
          ...
          instance.post(`admin/upcoming/store`, Data).then((response) => {
            ...
            if (status === true) {
              Swal.fire({
                title: 'Good job!',
                text: 'Daily Specials Manu Successfully Created',
                icon: 'success',
                button: 'Ok',
              });
              user()
    ...
    >
        <Box>
            ...
            <Box onClick={handleClose} sx={{ position: 'absolute', top: 10, right: 10, cursor: 'pointer' }}>
               ...
               {loader ? "Loading..." : "Add Daily Specials Manu"} 
          </Button>
            </Box>
        </Box>
    </Modal>
...
export default DailySpecialsModal;
