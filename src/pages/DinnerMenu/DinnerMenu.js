...
import DinnerMenuModal from './DinnerMenuModal';
import DinnerMenuModalEdit from './DinnerMenuModalEdit';
...
// edit
    ...
    try {
      instance.get(`admin/upcoming/delete/${id}`).then((response) => {
        if (response?.data.status === true) {
          ...
          Swal.fire({
            title: 'Good job! ',
            text: 'Dinner menu deleted successFully',
            icon: 'success',
            button: 'Ok',
          });
          user();
  ...
  return (
    <>
      <Helmet>
        <title>Dinner Menu | Ole brass rail </title>
      </Helmet>
...
{/* add  */}
<DinnerMenuModal open={open} setOpen={setOpen} handleClose={handleClose} user={user}/>
{/* edit */}
<DinnerMenuModalEdit EditData={EditData} openedit={openedit} setOpenedit={setOpenedit} handleCloseedit={handleCloseedit} user={user}/>
      </Container>
    </>
  );
}