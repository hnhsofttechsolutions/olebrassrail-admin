...
import BreakfastMenuEdit from './BreakfastMenuEdit';
...
// edit
    ...
    try {
      instance.get(`admin/upcoming/delete/${id}`).then((response) => {
        if (response?.data.status === true) {
          ...
          Swal.fire({
            title: 'Good job! ',
            text: 'Breakfast Menu deleted successFully',
            icon: 'success',
            button: 'Ok',
          });
          user();
  ...
  return (
    <>
      <Helmet>
        <title>Breakfast Menu | Ole brass rail </title>
      </Helmet>
...
{/* add  */}
<BreakfastMenuModal open={open} setOpen={setOpen} handleClose={handleClose} user={user}/>
{/* edit */}
<BreakfastMenuEdit EditData={EditData} openedit={openedit} setOpenedit={setOpenedit} handleCloseedit={handleCloseedit} user={user}/>
      </Container>
    </>
  );
}