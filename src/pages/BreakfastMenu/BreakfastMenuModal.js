...
const BreakfastMenuModal = ({ open,setOpen, handleClose,user }) => {
    ...
    const UpcomingEvent = () => {
        ...
        try {
          ...
          Data.append("tag_name","Breakfast Menu")
          ...
          instance.post(`admin/upcoming/store`, Data).then((response) => {
            ...
            if (status === true) {
              Swal.fire({
                title: 'Good job!',
                text: 'Breakfast Menu Successfully Created',
                icon: 'success',
                button: 'Ok',
              });
              user()
    ...
    >
        <Box>
            ...
            <Box onClick={handleClose} sx={{ position: 'absolute', top: 10, right: 10, cursor: 'pointer' }}>
               ...
               {loader ? "Loading..." : "Add Breakfast Menu"} 
          </Button>
            </Box>
        </Box>
    </Modal>
...
export default BreakfastMenuModal;
